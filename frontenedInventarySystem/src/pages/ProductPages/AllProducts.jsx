...
const AllProducts = () => {
  ...
  const fetchInventoryOverview = async () => {
    try {
      ...
      setStockError(null);
      const res = await axios.get(
        `http://localhost:5000/api/v1/products/getInventoryOverview`
      );
  ...
  const fetchProducts = useCallback(
    async (searchQuery = "", categoryId = null, stockStatus = null) => {
      try {
        ...
        let url = `http://localhost:5000/api/v1/products/getAllProducts`;
        let params = { q: searchQuery };
        
        if (categoryId) {
          url = `http://localhost:5000/api/v1/products/getProductsByCategory/${categoryId}`;
          params = {};
        }
